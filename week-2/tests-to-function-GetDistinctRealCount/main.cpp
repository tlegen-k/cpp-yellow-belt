...
void TestLinear() {
	...
	AssertEqual(GetDistinctRealRootCount(0, 1, 1), 1, "c=0");
}
void TestPureQuadratic() {
	AssertEqual(GetDistinctRealRootCount(0, 0, 1), 0, "a=0, b=0");
	AssertEqual(GetDistinctRealRootCount(1, 0, 0), 1, "b=0, c=0");
	AssertEqual(GetDistinctRealRootCount(1, 0, -1), 2, "b=0, -c/a > 0");
	AssertEqual(GetDistinctRealRootCount(-1, 0, 1), 2, "b=0, -c/a > 0");
	AssertEqual(GetDistinctRealRootCount(1, 0, 1), 0, "b=0, -c/a < 0");
	AssertEqual(GetDistinctRealRootCount(-1, 0, -1), 0, "b=0, -c/a < 0");
}
void TestFactored() {
	AssertEqual(GetDistinctRealRootCount(0, 1, 0), 1, "a=0, c=0");
	AssertEqual(GetDistinctRealRootCount(1, 0, 0), 1, "b=0, c=0");
	AssertEqual(GetDistinctRealRootCount(1, 1, 0), 2, "c=0");
}
void TestRootCount() {
	mt19937 gen;
...
int main() {
  TestRunner runner;
  runner.RunTest(TestLinear, "Testing linear equations");
  runner.RunTest(TestPureQuadratic, "Testing pure quadratic equations");
  runner.RunTest(TestFactored, "Testing factored quadratic equations");
