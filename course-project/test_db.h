...
public:
    bool Evaluate(const Date&, const std::string& event) const {
        return false;
    }
};
...
//     Last
    {
        ...
        db.Add(d1, "e1");
        db.Add(d2, "e2");
        AssertEqual(db.Last(d), pair<Date,string>({2018, 12, 22}, "e2"), "Db "
                                                                       "Last 1");
        Date d3(2018, 12, 24);
        db.Add(d3, "e3");
        AssertEqual(db.Last(d), pair<Date,string>({2018, 12, 24}, "e3"), "Db "
                                                                       "Last 2");
...
void TestPrint() {
        {
            ...
            ostringstream out;
            db.Print(out);
            AssertEqual("0001-01-01 xmas\n2017-01-01 new year\n", out.str(),
                        "filling with 0 in date");
        }
}
...
{
    Database db;
    db.Add({2017,01,01}, "event3");
    db.Add({2017,01,01}, "event2");
    db.Add({2017,01,01}, "event1");

    ostringstream out;
    db.Print(out);
    AssertEqual("2017-01-01 event3\n2017-01-01 event2\n2017-01-01 "
                "event1\n3",
                DoFind(db, "date == 2017-1-1"), "simple find by date");
}
